...
import type { OutputChunk, SourceMapInput } from 'rollup'
...
export function getElectronPath(): string {
  ...
  if (!electronExecPath) {
    const electronModulePath = getNpmModuleMainEntry('electron')
...
function getBytecodeCompilerPath(): string {
  const modulePath = path.resolve(getNpmModuleMainEntry('@marketing/vite-plugin'), 'src/bin', 'electron-bytecode.cjs')
  return modulePath
}
