...
import { drawNoise, drawNoiseToWebgl } from './utils';

export type HookTask = {
  name: string
  condition?: (fh: any) => boolean | undefined
  onEnable?: (fh: any) => void
}
const hookTaskMap: Record<string, Omit<HookTask, 'name'>> = {
  ...
  'hook navigator': {
    ...
    onEnable: ({ win, conf, _getSeed, getValue }) => {
    ...
    onEnable: ({ win, conf, getValueDebounce }) => {
      ...
      const getStandardDateTimeParts = (date: Date, timezone?: string): TimeParts | null => {
        ...
        try {
          ...
          return parst.reduce((acc: TimeParts, cur: any) => {
            acc[cur.type as keyof TimeParts] = cur.value
            return acc
          }, {})
        } catch (e) {
          return null
      ...
      /* DateTimeFormat */
      {
        win.Intl.DateTimeFormat = new Proxy(_DateTimeFormat, {
          construct: (target, args: Parameters<typeof Intl.DateTimeFormat>, _newTarget) => {
      ...
      /* Date */
      {
        win.Date = new Proxy(_Date, {
          apply: (target, _thisArg: Date, args: Parameters<typeof Date>) => {
    ...
    onEnable: ({ win, conf, getValueDebounce }) => {
      ...
      _offsetHeight && Object.defineProperty(win.HTMLElement.prototype, "offsetHeight", {
        get: new Proxy(_offsetHeight, {
          apply(_target, thisArg: HTMLElement, _args: any) {
            try {
              ...
              if (_data && _data instanceof Float32Array) {
                try {
                  ...
                  for (let i = 0; i < selected.length; i++) {
                    const index = selected[i] as number;
